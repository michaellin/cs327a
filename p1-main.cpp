...
void control(Sai2Model::Sai2Model* robot, Simulation::Sai2Simulation* sim) {
    ...
    // define desired pose and control parameters
    Eigen::Vector3d ddx;
    Eigen::VectorXd ddq = Eigen::VectorXd::Zero(robot->dof());
		switch (enum_problem_part) {
			case PART1:
        //ddx << kpx * (xd - ee_pos) - kvx * v;
        ddx = - kpx * (ee_x - ee_des_pos) - kvx * ee_dx;
        //ddx << - kvx * v;
        //tau = Jv.transpose()*(Lv*ddx + p) + robot->_M*(-kvj*robot->_dq);
        //tau = Jv.transpose()*(Lv*ddx + p) + robot->_M*(-kvj*robot->_dq);
        ...
        std::cout << "\n" << ddx << std::endl;
				break;
			case PART2:
        ddx << - kpx * (ee_pos - ee_des_pos);
        //ddx << - kvx * v;
        //tau = Jv.transpose()*(Lv*ddx + p) + robot->_M*(-kvj*robot->_dq);
        tau = Jv.transpose()*(p) + robot->_M*(-2*kvj*robot->_dq);
